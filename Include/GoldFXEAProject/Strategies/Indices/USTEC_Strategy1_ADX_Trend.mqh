...
/**
 * USTEC Strategy 1: ADX Trend Following (1H/4H)
 * Type: Trend Following
 * 
 * Rules:
 * 1. Long: ADX(14) > 25 AND +DI > -DI AND Close > EMA(50) AND EMA(50) > EMA(200).
 * 2. Short: ADX(14) > 25 AND -DI > +DI AND Close < EMA(50) AND EMA(50) < EMA(200).
 * 3. SL: 1.5 * ATR(14).
 * 4. TP: 3.0 * ATR(14).
 */
class CUSTEC_Strategy1_ADX_Trend : public CStrategyBase
{
private:
    int m_adxPeriod;
    int m_emaFast;
...
public:
    CUSTEC_Strategy1_ADX_Trend(CLogger* logger, CRiskManager* riskManager) 
        : CStrategyBase("USTEC_Strategy1_ADX_Trend", logger, riskManager)
    {
        m_adxPeriod = 14;
        m_emaFast = 50;
        m_emaSlow = 200;
    ...
    virtual TradeSignal GenerateSignal() override
    {
        ...
        // Long Entry
        if(adx[0] > 25 && diPlus[0] > diMinus[0] && close1 > emaF[0] && emaF[0] > emaS[0])
            return SIGNAL_BUY;
            
        // Short Entry
        if(adx[0] > 25 && diMinus[0] > diPlus[0] && close1 < emaF[0] && emaF[0] < emaS[0])
            return SIGNAL_SELL;
