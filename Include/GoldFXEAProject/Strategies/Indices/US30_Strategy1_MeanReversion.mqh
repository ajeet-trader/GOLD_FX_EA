//+------------------------------------------------------------------+
//|                                    US30_Strategy1_MeanReversion.mqh |
...
/**
 * US30 Strategy 1: Mean Reversion (Daily)
 * Type: Mean Reversion
 * 
 * Rules:
 * 1. Long: Close < MA(20) - (0.25 * ATR(14)) AND RSI(7) < 30 AND Close > Open.
 * 2. Short: Close > MA(20) + (0.25 * ATR(14)) AND RSI(7) > 70 AND Close < Open.
 * 3. SL: 1.5 * ATR(14).
 * 4. TP: MA(20).
 * 
 * Note: Smaller deviation (0.25 ATR) used for US30 due to lower volatility 
 * compared to US500/USTEC.
 */
class CUS30_Strategy1_MeanReversion : public CStrategyBase
{
private:
    int m_rsiPeriod;
    int m_maPeriod;
...
public:
    CUS30_Strategy1_MeanReversion(CLogger* logger, CRiskManager* riskManager) 
        : CStrategyBase("US30_Strategy1_MeanReversion", logger, riskManager)
    {
        m_rsiPeriod = 7;
        m_maPeriod = 20;
        m_atrPeriod = 14;
    ...
    virtual TradeSignal GenerateSignal() override
    {
        ...
        // Long Entry
        if(close1 < (ma[0] - 0.25 * atr[0]) && rsi[0] < 30 && close1 > open1)
            return SIGNAL_BUY;
            
        // Short Entry
        if(close1 > (ma[0] + 0.25 * atr[0]) && rsi[0] > 70 && close1 < open1)
            return SIGNAL_SELL;
