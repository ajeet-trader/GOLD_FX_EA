//+------------------------------------------------------------------+
//|                                  LTCUSD_Strategy2_BTC_Anchored.mqh |
...
/**
 * LTCUSD Strategy 2: BTC-Anchored Trend Following (4H)
 * Type: Beta-leveraged trend following
 * 
 * Concept:
 * - Only trade LTC in direction of BTC trend to reduce false moves.
 * - Use faster EMAs on LTC to capture higher beta.
 * 
 * Rules:
 * 1. BTC Trend: BTC-Close > BTC-EMA(50) AND BTC-EMA(20) > BTC-EMA(50) (Long bias).
 * 2. LTC Entry: Close > EMA(20) AND EMA(20) > EMA(50) AND break above last 10-bar high.
 * 3. SL: 2.0 * ATR(14).
 * 4. TP: 3.0 * ATR(14).
 */
class CLTCUSD_Strategy2_BTC_Anchored : public CStrategyBase
{
private:
    string m_btcSymbol;
    int m_emaFast;
...
public:
    CLTCUSD_Strategy2_BTC_Anchored(CLogger* logger, CRiskManager* riskManager) 
        : CStrategyBase("LTCUSD_Strategy2_BTC_Anchored", logger, riskManager)
    {
        m_btcSymbol = "BTCUSD";
        m_emaFast = 20;
        m_emaSlow = 50;
    ...
    virtual bool Initialize() override
    {
        ...
        if(m_config.symbol == "")
        {
            ...
            m_config.magicNumber = EA_MAGIC_NUMBER + 407;
        }
        ...
        // Indicators for BTC
        m_hBtcEmaFast = iMA(m_btcSymbol, m_config.timeframe, m_btcEmaFast, 0, MODE_EMA, PRICE_CLOSE);
        m_hBtcEmaSlow = iMA(m_btcSymbol, m_config.timeframe, m_btcEmaSlow, 0, MODE_EMA, PRICE_CLOSE);
        ...
        if(m_handleEMA_Fast == INVALID_HANDLE || m_handleEMA_Slow == INVALID_HANDLE || 
           ...
           m_hBtcEmaSlow == INVALID_HANDLE)
        {
            m_logger.Error("Failed to initialize indicators for anchored trend", m_moduleName);
            return false;
        }
    ...
    virtual TradeSignal GenerateSignal() override
    {
        ...
        // Get LTC data
        double ltcEmaF[], ltcEmaS[], ltcAtr[];
        if(CopyBuffer(m_handleEMA_Fast, 0, 1, 1, ltcEmaF) < 1 ||
           CopyBuffer(m_handleEMA_Slow, 0, 1, 1, ltcEmaS) < 1 ||
           CopyBuffer(m_handleATR, 0, 1, 1, ltcAtr) < 1) return signal;
        ...
        double ltcClose = iClose(m_config.symbol, m_config.timeframe, 1);
        double btcClose = iClose(m_btcSymbol, m_config.timeframe, 1);
        
        // LTC High/Low for breakout
        double ltcHigh = iHigh(m_config.symbol, m_config.timeframe, iHighest(m_config.symbol, m_config.timeframe, MODE_HIGH, m_lookback, 1));
        double ltcLow = iLow(m_config.symbol, m_config.timeframe, iLowest(m_config.symbol, m_config.timeframe, MODE_LOW, m_lookback, 1));
        
        // Long Conditions
        bool btcTrendLong = btcClose > btcEmaS[0] && btcEmaF[0] > btcEmaS[0];
        bool ltcTrendLong = ltcClose > ltcEmaF[0] && ltcEmaF[0] > ltcEmaS[0];
        bool ltcBreakLong = ltcClose > ltcHigh;
        
        if(btcTrendLong && ltcTrendLong && ltcBreakLong)
        {
            ...
            signal.reason = "LTC-2: BTC-Anchored Trend Following";
            return signal;
        }
        
        // Short Conditions
        bool btcTrendShort = btcClose < btcEmaS[0] && btcEmaF[0] < btcEmaS[0];
        bool ltcTrendShort = ltcClose < ltcEmaF[0] && ltcEmaF[0] < ltcEmaS[0];
        bool ltcBreakShort = ltcClose < ltcLow;
        
        if(btcTrendShort && ltcTrendShort && ltcBreakShort)
        {
