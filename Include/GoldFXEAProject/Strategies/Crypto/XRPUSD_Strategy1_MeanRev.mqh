...
//+------------------------------------------------------------------+
//| XRPUSD Strategy 1: Range-Bound Mean Reversion (4H)               |
//+------------------------------------------------------------------+
class CXRPUSD_Strategy1_MeanRev : public CStrategyBase
{
private:
    int m_bbPeriod;
    double m_bbDev;
...
public:
    CXRPUSD_Strategy1_MeanRev(CLogger* logger, CRiskManager* riskManager) 
        : CStrategyBase("XRPUSD_Strategy1_MeanRev", logger, riskManager)
    {
        m_bbPeriod = 20;
        m_bbDev = 2.0;
        m_rsiPeriod = 14;
    ...
    virtual bool Initialize() override
    {
        ...
        m_handleBB = CreateIndicator("BB_20_2_4H", "BollingerBands", m_bbPeriod, 0, m_bbDev);
    ...
    virtual TradeSignal GenerateSignal() override
    {
        ...
        if(closePrice < bbLower && rsi < 30)
        {
             ...
             signal.reason = "XRP-1: BB + RSI Mean Reversion (4H)";
             return signal;
        }
    ...
    virtual void CalculateTakeProfit(double entryPrice, ENUM_ORDER_TYPE orderType, double &takeProfit) override { takeProfit = m_bbMain[1]; }
